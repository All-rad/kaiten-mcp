...
- Q: Где предпочтительнее хранить токен и профиль? → A: Custom — SQLite (локальная `data/kaiten.db`, открытое хранение)
- Q: Предпочтительная модель хранения учётных данных? → A: A (Single credential — сохраняется одна запись)
...
После успешной проверки токен и профиль должны сохраняться локально (в открытом виде, как указано в требованиях) и автоматически загружаться при следующем запуске приложения, при этом профиль обновляется повторным запросом к Kaiten при старте. Приложение поддерживает один активный набор учётных данных: новая авторизация перезаписывает ранее сохранённые данные (MVP).
